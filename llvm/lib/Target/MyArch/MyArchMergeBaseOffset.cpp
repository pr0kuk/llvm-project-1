//===----- MyArchMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "myarch-merge-base-offset"
#define MyArch_MERGE_BASE_OFFSET_NAME "MyArch Merge Base Offset"
namespace {

struct MyArchMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  MyArchMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return MyArch_MERGE_BASE_OFFSET_NAME;
  }
...
char MyArchMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(MyArchMergeBaseOffsetOpt, DEBUG_TYPE,
                MyArch_MERGE_BASE_OFFSET_NAME, false, false)
...
bool MyArchMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != MyArch::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != MyArchII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != MyArch::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != MyArchII::MO_LO ||
...
// output from LoADDI.
void MyArchMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool MyArchMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == MyArch::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == MyArch::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != MyArchII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != MyArch::LUI ||
        LuiImmOp.getTargetFlags() != MyArchII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == MyArch::LUI) {
...
bool MyArchMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool MyArchMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createMyArchMergeBaseOffsetOptPass() {
  return new MyArchMergeBaseOffsetOpt();
}
