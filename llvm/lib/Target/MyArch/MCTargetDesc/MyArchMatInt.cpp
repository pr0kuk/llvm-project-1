//===- MyArchMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
void generateInstSeq(int64_t Val, bool IsRV64, InstSeq &Res) {
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(Inst(MyArch::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (IsRV64 && Hi20) ? MyArch::ADDIW : MyArch::ADDI;
  ...
  Res.push_back(Inst(MyArch::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(Inst(MyArch::ADDI, Lo12));
}
