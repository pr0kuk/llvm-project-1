//===-- MyArchBaseInfo.h - Top level definitions for MyArch MC ----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains small standalone enum definitions for the MyArch target
// useful for the compiler back-end and the MC libraries.
//
//===----------------------------------------------------------------------===//
#ifndef LLVM_LIB_TARGET_MyArch_MCTARGETDESC_MyArchBASEINFO_H
#define LLVM_LIB_TARGET_MyArch_MCTARGETDESC_MyArchBASEINFO_H
...
// MyArchII - This namespace holds all of the target specific flags that
// instruction info tracks. All definitions must match MyArchInstrFormats.td.
namespace MyArchII {
enum {
  InstFormatPseudo = 0,
  InstFormatR = 1,
...
// Match with the definitions in MyArchInstrFormatsV.td
...
enum OperandType : unsigned {
  OPERAND_FIRST_MyArch_IMM = MCOI::OPERAND_FIRST_TARGET,
  OPERAND_UIMM4 = OPERAND_FIRST_MyArch_IMM,
  OPERAND_UIMM5,
  OPERAND_UIMM12,
  OPERAND_SIMM12,
  OPERAND_UIMM20,
  OPERAND_UIMMLOG2XLEN,
  OPERAND_LAST_MyArch_IMM = OPERAND_UIMMLOG2XLEN
};
} // namespace MyArchOp

// Describes the predecessor/successor bits used in the FENCE instruction.
namespace MyArchFenceField {
enum FenceField {
  I = 8,
  O = 4,
  R = 2,
...
inline static StringRef roundingModeToString(RoundingMode RndMode) {
  ...
  case MyArchFPRndMode::RNE:
    return "rne";
  case MyArchFPRndMode::RTZ:
    return "rtz";
  case MyArchFPRndMode::RDN:
    return "rdn";
  case MyArchFPRndMode::RUP:
    return "rup";
  case MyArchFPRndMode::RMM:
    return "rmm";
  case MyArchFPRndMode::DYN:
    return "dyn";
  }
}
...
inline static RoundingMode stringToRoundingMode(StringRef Str) {
  return StringSwitch<RoundingMode>(Str)
      .Case("rne", MyArchFPRndMode::RNE)
      .Case("rtz", MyArchFPRndMode::RTZ)
      .Case("rdn", MyArchFPRndMode::RDN)
      .Case("rup", MyArchFPRndMode::RUP)
      .Case("rmm", MyArchFPRndMode::RMM)
      .Case("dyn", MyArchFPRndMode::DYN)
      .Default(MyArchFPRndMode::Invalid);
}
...
struct SysReg {
  ...
  bool haveRequiredFeatures(FeatureBitset ActiveFeatures) const {
    // Not in 32-bit mode.
    if (isRV32Only && ActiveFeatures[MyArch::Feature64Bit])
      return false;
...
inline static unsigned encodeVTYPE(MyArchVLMUL VLMUL, MyArchVSEW VSEW,
...
inline static MyArchVLMUL getVLMUL(unsigned VType) {
  unsigned VLMUL = VType & 0x7;
  return static_cast<MyArchVLMUL>(VLMUL);
}

inline static MyArchVSEW getVSEW(unsigned VType) {
  unsigned VSEW = (VType >> 3) & 0x7;
  return static_cast<MyArchVSEW>(VSEW);
}
...
#define GET_MyArchVPseudosTable_DECL
#include "MyArchGenSearchableTables.inc"

} // end namespace MyArchVPseudosTable
