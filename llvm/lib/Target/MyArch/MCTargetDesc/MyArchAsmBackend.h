//===-- MyArchAsmBackend.h - MyArch Assembler Backend -----------------------===//
//
...
class MyArchAsmBackend : public MCAsmBackend {
  ...
  MyArchABI::ABI TargetABI = MyArchABI::ABI_Unknown;

public:
  MyArchAsmBackend(const MCSubtargetInfo &STI, uint8_t OSABI, bool Is64Bit,
                  const MCTargetOptions &Options)
      : MCAsmBackend(support::little), STI(STI), OSABI(OSABI), Is64Bit(Is64Bit),
        TargetOptions(Options) {
    TargetABI = MyArchABI::computeTargetABI(
        STI.getTargetTriple(), STI.getFeatureBits(), Options.getABIName());
    MyArchFeatures::validate(STI.getTargetTriple(), STI.getFeatureBits());
  }
  ~MyArchAsmBackend() override {}
  ...
  bool willForceRelocations() const {
    return ForceRelocs || STI.getFeatureBits()[MyArch::FeatureRelax];
  }
  ...
  MyArchABI::ABI getTargetABI() const { return TargetABI; }
};
}
