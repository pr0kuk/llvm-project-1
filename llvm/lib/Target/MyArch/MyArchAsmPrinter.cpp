//===-- MyArchAsmPrinter.cpp - MyArch LLVM assembly writer ------------------===//
//
...
// of machine-dependent LLVM code to the MyArch assembly language.
//
//===----------------------------------------------------------------------===//
...
void MyArchAsmPrinter::EmitToStreamer(MCStreamer &S, const MCInst &Inst) {
  MCInst CInst;
...
void MyArchAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  MCInst TmpInst;
  LowerMyArchMachineInstrToMCInst(MI, TmpInst, *this);
  EmitToStreamer(*OutStreamer, TmpInst);
}

bool MyArchAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
  ...
  if (ExtraCode && ExtraCode[0]) {
    ...
    case 'z':      // Print zero register if zero, regular printing otherwise.
      if (MO.isImm() && MO.getImm() == 0) {
        OS << MyArchInstPrinter::getRegisterName(MyArch::X0);
        return false;
      }
      break;
  ...
  case MachineOperand::MO_Register:
    OS << MyArchInstPrinter::getRegisterName(MO.getReg());
    return false;
...
bool MyArchAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
                                            unsigned OpNo,
  ...
  if (!ExtraCode) {
    ...
    OS << "0(" << MyArchInstPrinter::getRegisterName(MO.getReg()) << ")";
    return false;
  }
...
void MyArchAsmPrinter::emitEndOfAsmFile(Module &M) {
  MyArchTargetStreamer &RTS =
      static_cast<MyArchTargetStreamer &>(*OutStreamer->getTargetStreamer());

  if (TM.getTargetTriple().isOSBinFormatELF())
    RTS.finishAttributeSection();
}

void MyArchAsmPrinter::emitAttributes() {
  MyArchTargetStreamer &RTS =
      static_cast<MyArchTargetStreamer &>(*OutStreamer->getTargetStreamer());
  ...
  const MyArchTargetMachine &RTM = static_cast<const MyArchTargetMachine &>(TM);
  ...
  const MyArchSubtarget STI(TT, CPU, /*TuneCPU=*/CPU, FS, /*ABIName=*/"", RTM);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeMyArchAsmPrinter() {
  ...
  //RegisterAsmPrinter<MyArchAsmPrinter> X(getTheMyArch32Target());
  //RegisterAsmPrinter<MyArchAsmPrinter> Y(getTheMyArch64Target());
}
