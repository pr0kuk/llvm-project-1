//===-- MyArchExpandAtomicPseudoInsts.cpp - Expand atomic pseudo instrs. ---===//
//
...
#define MyArch_EXPAND_ATOMIC_PSEUDO_NAME                                        \
  "MyArch atomic pseudo instruction expansion pass"

namespace {

class MyArchExpandAtomicPseudo : public MachineFunctionPass {
public:
  const MyArchInstrInfo *TII;
  static char ID;

  MyArchExpandAtomicPseudo() : MachineFunctionPass(ID) {
    initializeMyArchExpandAtomicPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return MyArch_EXPAND_ATOMIC_PSEUDO_NAME;
  }
...
char MyArchExpandAtomicPseudo::ID = 0;

bool MyArchExpandAtomicPseudo::runOnMachineFunction(MachineFunction &MF) {
...
bool MyArchExpandAtomicPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool MyArchExpandAtomicPseudo::expandMI(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MBBI,
                                       MachineBasicBlock::iterator &NextMBBI) {
...
static unsigned getLRForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return MyArch::LR_W_AQ;
  case AtomicOrdering::Release:
    return MyArch::LR_W;
  case AtomicOrdering::AcquireRelease:
    return MyArch::LR_W_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return MyArch::LR_W_AQ_RL;
  }
}
...
static unsigned getSCForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return MyArch::SC_W;
  case AtomicOrdering::Release:
    return MyArch::SC_W_RL;
  case AtomicOrdering::AcquireRelease:
    return MyArch::SC_W_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return MyArch::SC_W_AQ_RL;
  }
}
...
static unsigned getLRForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Monotonic:
    return MyArch::LR_D;
  case AtomicOrdering::Acquire:
    return MyArch::LR_D_AQ;
  case AtomicOrdering::Release:
    return MyArch::LR_D;
  case AtomicOrdering::AcquireRelease:
    return MyArch::LR_D_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return MyArch::LR_D_AQ_RL;
  }
}
...
static unsigned getSCForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return MyArch::SC_D;
  case AtomicOrdering::Release:
    return MyArch::SC_D_RL;
  case AtomicOrdering::AcquireRelease:
    return MyArch::SC_D_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return MyArch::SC_D_AQ_RL;
  }
}
...
static void doAtomicBinOpExpansion(const MyArchInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(MyArch::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(MyArch::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const MyArchInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(MyArch::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(MyArch::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(MyArch::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const MyArchInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(MyArch::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(MyArch::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(MyArch::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool MyArchExpandAtomicPseudo::expandAtomicBinOp(
...
static void insertSext(const MyArchInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(MyArch::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(MyArch::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool MyArchExpandAtomicPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(MyArch::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(MyArch::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(MyArch::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(MyArch::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(MyArch::BNE))
      .addReg(Scratch1Reg)
      .addReg(MyArch::X0)
      .addMBB(LoopHeadMBB);
...
bool MyArchExpandAtomicPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(MyArch::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(MyArch::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(MyArch::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(MyArchExpandAtomicPseudo, "myarch-expand-atomic-pseudo",
                MyArch_EXPAND_ATOMIC_PSEUDO_NAME, false, false)

namespace llvm {

FunctionPass *createMyArchExpandAtomicPseudoPass() {
  return new MyArchExpandAtomicPseudo();
}
