//===-- MyArchFrameLowering.cpp - MyArch Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the MyArch implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
static void emitSCSPrologue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  Register SCSPReg = MyArchABI::getSCSPReg();
  ...
  const MyArchInstrInfo *TII = STI.getInstrInfo();
  bool IsRV64 = STI.hasFeature(MyArch::Feature64Bit);
  int64_t SlotSize = STI.getXLen() / 8;
  // Store return address to shadow call stack
  // s[w|d]  ra, 0(s2)
  // addi    s2, s2, [4|8]
  BuildMI(MBB, MI, DL, TII->get(IsRV64 ? MyArch::SD : MyArch::SW))
      .addReg(RAReg)
      .addReg(SCSPReg)
      .addImm(0);
  BuildMI(MBB, MI, DL, TII->get(MyArch::ADDI))
...
static void emitSCSEpilogue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  // l[w|d]  ra, -[4|8](s2)
  // addi    s2, s2, -[4|8]
  BuildMI(MBB, MI, DL, TII->get(IsRV64 ? MyArch::LD : MyArch::LW))
...
static int getLibCallID(const MachineFunction &MF,
  ...
  Register MaxReg = MyArch::NoRegister;
  for (auto &CS : CSI)
    // MyArchRegisterInfo::hasReservedSpillSlot assigns negative frame indexes to
    // registers which can be saved by libcall.
    if (CS.getFrameIdx() < 0)
      MaxReg = std::max(MaxReg.id(), CS.getReg().id());

  if (MaxReg == MyArch::NoRegister)
    return -1;

  switch (MaxReg) {
  default:
    llvm_unreachable("Something has gone wrong!");
  case /*s11*/ MyArch::X27: return 12;
  case /*s10*/ MyArch::X26: return 11;
  case /*s9*/  MyArch::X25: return 10;
  case /*s8*/  MyArch::X24: return 9;
  case /*s7*/  MyArch::X23: return 8;
  case /*s6*/  MyArch::X22: return 7;
  case /*s5*/  MyArch::X21: return 6;
  case /*s4*/  MyArch::X20: return 5;
  case /*s3*/  MyArch::X19: return 4;
  case /*s2*/  MyArch::X18: return 3;
  case /*s1*/  MyArch::X9:  return 2;
  case /*s0*/  MyArch::X8:  return 1;
  case /*ra*/  MyArch::X1:  return 0;
  }
}
...
bool MyArchFrameLowering::hasFP(const MachineFunction &MF) const {
...
bool MyArchFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void MyArchFrameLowering::determineFrameLayout(MachineFunction &MF) const {
...
void MyArchFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(MyArch::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
    unsigned Opc = MyArch::ADD;
    bool isSub = Val < 0;
    if (isSub) {
      Val = -Val;
      Opc = MyArch::SUB;
...
static Register getFPReg(const MyArchSubtarget &STI) { return MyArch::X8; }

// Returns the register used to hold the stack pointer.
static Register getSPReg(const MyArchSubtarget &STI) { return MyArch::X2; }
...
void MyArchFrameLowering::emitPrologue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<MyArchMachineFunctionInfo>();
  const MyArchRegisterInfo *RI = STI.getRegisterInfo();
  const MyArchInstrInfo *TII = STI.getInstrInfo();
  MachineBasicBlock::iterator MBBI = MBB.begin();

  Register FPReg = getFPReg(STI);
  Register SPReg = getSPReg(STI);
  Register BPReg = MyArchABI::getBPReg();
  ...
  if (hasFP(MF)) {
    ...
    if (RI->needsStackRealignment(MF)) {
      ...
      if (isInt<12>(-(int)MaxAlignment.value())) {
        BuildMI(MBB, MBBI, DL, TII->get(MyArch::ANDI), SPReg)
            .addReg(SPReg)
            .addImm(-(int)MaxAlignment.value());
      } else {
        unsigned ShiftAmount = Log2(MaxAlignment);
        Register VR =
            MF.getRegInfo().createVirtualRegister(&MyArch::GPRRegClass);
        BuildMI(MBB, MBBI, DL, TII->get(MyArch::SRLI), VR)
            .addReg(SPReg)
            .addImm(ShiftAmount);
        BuildMI(MBB, MBBI, DL, TII->get(MyArch::SLLI), SPReg)
            .addReg(VR)
            .addImm(ShiftAmount);
      }
      ...
      if (hasBP(MF)) {
        // move BP, SP
        BuildMI(MBB, MBBI, DL, TII->get(MyArch::ADDI), BPReg)
            .addReg(SPReg)
            .addImm(0);
      }
    }
...
void MyArchFrameLowering::emitEpilogue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  const MyArchRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<MyArchMachineFunctionInfo>();
...
StackOffset
MyArchFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
...
void MyArchFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(MyArch::X1);
    SavedRegs.set(MyArch::X8);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(MyArchABI::getBPReg());
  ...
  if (MF.getFunction().hasFnAttribute("interrupt") && MFI.hasCalls()) {
    ...
    static const MCPhysReg CSRegs[] = { MyArch::X1,      /* ra */
      MyArch::X5, MyArch::X6, MyArch::X7,                  /* t0-t2 */
      MyArch::X10, MyArch::X11,                           /* a0-a1, a2-a7 */
      MyArch::X12, MyArch::X13, MyArch::X14, MyArch::X15, MyArch::X16, MyArch::X17,
      MyArch::X28, MyArch::X29, MyArch::X30, MyArch::X31, 0 /* t3-t6 */
    };

    for (unsigned i = 0; CSRegs[i]; ++i)
      SavedRegs.set(CSRegs[i]);

    if (MF.getSubtarget<MyArchSubtarget>().hasStdExtF()) {

      // If interrupt is enabled, this list contains all FP registers.
      const MCPhysReg * Regs = MF.getRegInfo().getCalleeSavedRegs();

      for (unsigned i = 0; Regs[i]; ++i)
        if (MyArch::FPR16RegClass.contains(Regs[i]) ||
            MyArch::FPR32RegClass.contains(Regs[i]) ||
            MyArch::FPR64RegClass.contains(Regs[i]))
          SavedRegs.set(Regs[i]);
    }
  }
}

void MyArchFrameLowering::processFunctionBeforeFrameFinalized(
...
bool MyArchFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
  return !MF.getFrameInfo().hasVarSizedObjects();
}

// Eliminate ADJCALLSTACKDOWN, ADJCALLSTACKUP pseudo instructions.
MachineBasicBlock::iterator MyArchFrameLowering::eliminateCallFramePseudoInstr(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MI) const {
  Register SPReg = MyArch::X2;
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == MyArch::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
//   add     sp,sp,-64
uint64_t
MyArchFrameLowering::getFirstSPAdjustAmount(const MachineFunction &MF) const {
  const auto *RVFI = MF.getInfo<MyArchMachineFunctionInfo>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
bool MyArchFrameLowering::spillCalleeSavedRegisters(
  ...
  if (SpillLibCall) {
    ...
    BuildMI(MBB, MI, DL, TII.get(MyArch::PseudoCALLReg), MyArch::X5)
        .addExternalSymbol(SpillLibCall, MyArchII::MO_CALL)
...
bool MyArchFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
  ...
  if (RestoreLibCall) {
    ...
    MachineBasicBlock::iterator NewMI =
        BuildMI(MBB, MI, DL, TII.get(MyArch::PseudoTAIL))
            .addExternalSymbol(RestoreLibCall, MyArchII::MO_CALL)
            .setMIFlag(MachineInstr::FrameDestroy);

    // Remove trailing returns, since the terminator is now a tail call to the
    // restore function.
    if (MI != MBB.end() && MI->getOpcode() == MyArch::PseudoRET) {
...
bool MyArchFrameLowering::canUseAsPrologue(const MachineBasicBlock &MBB) const {
  ...
  // Inserting a call to a __myarch_save libcall requires the use of the register
  ...
  return !RS.isRegUsed(MyArch::X5);
}

bool MyArchFrameLowering::canUseAsEpilogue(const MachineBasicBlock &MBB) const {
  const MachineFunction *MF = MBB.getParent();
  MachineBasicBlock *TmpMBB = const_cast<MachineBasicBlock *>(&MBB);
  const auto *RVFI = MF->getInfo<MyArchMachineFunctionInfo>();
  ...
  // Using the __myarch_restore libcalls to restore CSRs requires a tail call.
