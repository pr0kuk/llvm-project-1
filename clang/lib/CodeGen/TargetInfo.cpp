...
// //===----------------------------------------------------------------------===//
// // MyArch ABI Implementation
// //===----------------------------------------------------------------------===//

// namespace {
// class MyArchABIInfo : public DefaultABIInfo {
// private:
//   // Size of the integer ('x') registers in bits.
//   unsigned XLen;
//   // Size of the floating point ('f') registers in bits. Note that the target
...
// public:
//   MyArchABIInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen, unsigned FLen)
//       : DefaultABIInfo(CGT), XLen(XLen), FLen(FLen) {}
...
// void MyArchABIInfo::computeInfo(CGFunctionInfo &FI) const {
//   QualType RetTy = FI.getReturnType();
//   if (!getCXXABI().classifyReturnType(FI))
//     FI.getReturnInfo() = classifyReturnType(RetTy);
...
// // responsible for checking that if there is only a single field then that
// // field is a float.
// bool MyArchABIInfo::detectFPCCEligibleStructHelper(QualType Ty, CharUnits CurOff,
//                                                   llvm::Type *&Field1Ty,
//                                                   CharUnits &Field1Off,
//                                                   llvm::Type *&Field2Ty,
//                                                   CharUnits &Field2Off) const {
...
// // value, fp+fp, or int+fp of appropriate size). If so, NeededArgFPRs and
// // NeededArgGPRs are incremented appropriately.
// bool MyArchABIInfo::detectFPCCEligibleStruct(QualType Ty, llvm::Type *&Field1Ty,
//                                             CharUnits &Field1Off,
//                                             llvm::Type *&Field2Ty,
//                                             CharUnits &Field2Off,
//                                             int &NeededArgGPRs,
...
// // Field1Ty, Field1Off, Field2Ty, Field2Off. This method will create an
// // appropriate coerceToType and unpaddedCoerceToType.
// ABIArgInfo MyArchABIInfo::coerceAndExpandFPCCEligibleStruct(
//     llvm::Type *Field1Ty, CharUnits Field1Off, llvm::Type *Field2Ty,
//     CharUnits Field2Off) const {
//   SmallVector<llvm::Type *, 3> CoerceElts;
//   SmallVector<llvm::Type *, 2> UnpaddedCoerceElts;
...
// ABIArgInfo MyArchABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
//                                               int &ArgGPRsLeft,
//                                               int &ArgFPRsLeft) const {
//   assert(ArgGPRsLeft <= NumArgGPRs && "Arg GPR tracking underflow");
//   Ty = useFirstFieldIfTransparentUnion(Ty);
...
// ABIArgInfo MyArchABIInfo::classifyReturnType(QualType RetTy) const {
//   if (RetTy->isVoidType())
//     return ABIArgInfo::getIgnore();
...
// Address MyArchABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
//                                 QualType Ty) const {
//   CharUnits SlotSize = CharUnits::fromQuantity(XLen / 8);
...
// ABIArgInfo MyArchABIInfo::extendType(QualType Ty) const {
//   int TySize = getContext().getTypeSize(Ty);
//   // RV64 ABI requires unsigned 32 bit integers to be sign extended.
//   if (XLen == 64 && Ty->isUnsignedIntegerOrEnumerationType() && TySize == 32)
//     return ABIArgInfo::getSignExtend(Ty);
...
// namespace {
// class MyArchTargetCodeGenInfo : public TargetCodeGenInfo {
// public:
//   MyArchTargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen,
//                          unsigned FLen)
//       : TargetCodeGenInfo(std::make_unique<MyArchABIInfo>(CGT, XLen, FLen)) {}
...
//     const auto *Attr = FD->getAttr<MyArchInterruptAttr>();
//     if (!Attr)
//       return;

//     const char *Kind;
//     switch (Attr->getInterrupt()) {
//     case MyArchInterruptAttr::user: Kind = "user"; break;
//     case MyArchInterruptAttr::supervisor: Kind = "supervisor"; break;
//     case MyArchInterruptAttr::machine: Kind = "machine"; break;
//     }
